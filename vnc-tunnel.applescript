...
display dialog "SSH (Gateway) Host:" default answer ""
set gateway_host to the text returned of the result

display dialog "VNC (Target) Host:" default answer ""
...
do shell script "echo " & quoted form of gateway_host & Â
	" | grep -ivq '[^-a-z0-9_.@]'"
do shell script "echo " & quoted form of target_host & Â
	" | grep -ivq '[^-a-z0-9_.@]'"

set stored_name to quoted form of gateway_host & ":" & target_host
...
do shell script "export H=" & quoted form of stored_name & Â
...
repeat 3 times
	try
		...
		do shell script "SSHVNC_DEST=" & quoted form of gateway_host & Â
			...
			":" & target_host & ":" & vnc_port & Â
			" -o ExitOnForwardFailure=yes -n -f " & quoted form of gateway_host & Â
